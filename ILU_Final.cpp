...
for (int i = 0; i < m; i++)
    {
        ...
        {   
            ...
            if (j > 0)    Aw[idx] -= 1.0; // West neighbor
            if (j < m-1) Ae[idx] -= 1.0;  // East neighbor
            if (i > 0)    As[idx] -= 1.0; // South neighbor
            if (i < m-1) An[idx] -= 1.0; // North neighbor
            
            // Adjust boundary conditions
            if (i == 0 || i == m - 1) Ap[idx] += 1.0;
            if (j == 0 || j == m - 1) Ap[idx] += 1.0;
...
int main()
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        {
            Lp[i] = Ap[i] - Ls[i] * Un[i - 1] - Lw[i] * Ue[i - m];
        }
    ...
    // for (int i = 0; i < n; i++)
    // {
    //     file << i << "  " << Aw[i] << "  " << As[i] << "  " << Ap[i] << "  " << An[i] << "  " << Ae[i] << endl;
    // }
    ...
    // Save results
    save_results(x, "temperature_cg_PCG.dat");
    save_centerlines(x, "T_centerline_H_PCG.dat", "T_centerline_V_PCG.dat");
    ...
    save_results(T_analytical, "temperature_analytical_PCG.dat");
    ...
    ofstream res_file("residual_history_PCG.dat");
    ...
    // Matrix_P_inv_Vect_Multi(x, temp_2);
    // file << endl;
    // for (int i = 0; i < n; i++)
    // {

    //     file << i << "  " << temp_2[i] << endl;
    // }
